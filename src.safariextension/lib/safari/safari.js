...
app.context_menu = (function () {
  ...
  safari.application.addEventListener('contextmenu', function (e) {
    onPage.forEach(function (arr, i) {
      if (arr[2] !== 'img' && e.userInfo.nodeName === 'IMG' || arr[2] === 'img' && e.userInfo.nodeName !== 'IMG') {
        return;
      }
      e.contextMenu.appendContextMenuItem('contextmenu' + i, arr[0], i + (e.userInfo.src || ''));
    });
  }, false);
  safari.application.addEventListener('command', function (e) {
    onPage[+e.command.substr(0, 1)][3](e.command.substr(1));
  }, false);

  return {
    create: function (label, img, type, callback) {
      onPage.push([label, img, type, callback]);
    }
  };
})();
